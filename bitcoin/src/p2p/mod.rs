...
macro_rules! generate_network_magic_conversion {
    ($($network_pat:pat => $magic:ident => $network_expr:expr,)*) => {
        impl From<Network> for Magic {
            fn from(network: Network) -> Magic {
                match network {
                    $(
                        $network_pat => Magic::$magic,
                    )*
                }
            }
        }
...
generate_network_magic_conversion! {
    ...
    Network::Testnet(TestnetVersion::V3) => TESTNET => Network::Testnet(TestnetVersion::V3),
    Network::Testnet(TestnetVersion::V4) => TESTNET4 => Network::Testnet(TestnetVersion::V4),
    Network::Signet => SIGNET => Network::Signet,
    Network::Regtest => REGTEST => Network::Regtest,
}
